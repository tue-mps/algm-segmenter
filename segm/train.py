...
def main(
    ...
    variant = dict(
        ...
        dataset_kwargs=dict(
            ...
            threshold=threshold,
            merging_window_size=merging_window_size,
        ...
        optimizer_kwargs=dict(
            ...
            sched=scheduler,
            epochs=num_epochs,
            min_lr= minlearning_rate, # default value 1e-5,
            poly_power=0.9,
            poly_step_size=1,
        ),
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if eval_epoch:
            ...
            print(str(eval_logger.mean_iou).split(" ")[0])
            print("")

            try:
                curr_checkpoint = str(eval_logger.mean_iou).split(" ")[0]
                if float(curr_checkpoint) > best_checkpoint:
                    print("saving the best checkpoint ...")
                    best_checkpoint = float(curr_checkpoint)
                    best_checkpoint_name = "best_checkpoint_" + str(best_checkpoint) + ".pth"
